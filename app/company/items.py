...
@router.post("/create",
             ...
             response_model=schemas.CreateCompanyDto,
             status_code=status.HTTP_201_CREATED,
             responses={
                 201: {"description": "Компания успешно создана"},
                 400: {"description": "Ошибка при создании компании"}
             })
async def create_new_company(company_data: schemas.Company,
                             db: AsyncSession = Depends(get_db)):
    """
...
async def patch_company(company_id: int,
    ...
    """
        ...
        """
    return await crud.update_company_data(db, company_id=company_id, company_data=updated_company,
...
async def update_status(
    ...
    db_company = await crud.update_company_status(db, company_id=company_id, is_active=company_status.is_active,
                                                  client_token=client_token)

    logger.info(f"Статус компании {company_id} успешно обновлен на {db_company.is_active}")
